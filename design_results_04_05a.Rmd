...
fout <- list.files("out05a")

for(i in 1:length(fout)){
  l <- readRDS(paste0("out05a/", fout[i]))
  ...
  dtmp <- cbind(l$dni, apply(l$dni[, 1:ncol(l$dni)], 2, function(x){ x > 0.5}))
...
kable(drecov,
      ...
      col.names = c("Scenario", "Range", "Mid","n(dose)", "min(dose)", "max(dose)",  "n_per_arm", 
                    comparison_names),
      digits = digits) %>%
...
The figure below shows the estimated (black) proportion recovered.

```{r, echo = F, fig.width=6, fig.height=6, fig.cap="Estimated and true probability of recovery", fig.align="center"}
