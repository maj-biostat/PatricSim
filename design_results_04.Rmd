...
fout <- list.files("out10")

for(i in 1:nres){
  l <- readRDS(paste0("out10/", fout[i]))
  ...
  # non-inferiority
  dtmp <- cbind(l$dni, apply(l$dni[, 1:ncol(l$dni)], 2, function(x){ x > 0.8}))
...
comparison_names <- paste0(0:10, " days")
...
dfig1 <- drecov %>%
  dplyr::select(-n_per_trt) %>%
  tidyr::gather("dose", "precov", -range, -scenario) %>%
  dplyr::arrange(scenario, dose) %>%
  dplyr::mutate(dose = gsub("theta\\[", "", dose),
    dose = gsub("\\]", "", dose), 
...
comparison_names <- paste0(0:9, " days")
...
kable(dsup,
  ...
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                ...
                font_size = 12,
                latex_options = "hold_position")  %>%
  add_header_above(c("Scenario" = 1, "Range" = 1, "N" = 1,
    "10 days versus" = 10))
...
An 80% decision threshold was used.
